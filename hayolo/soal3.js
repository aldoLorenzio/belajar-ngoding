...
function ganjilGenapAsianGames(date, data) {
    ...
    if(date >= 1 && date <= 31){
        for(let i = 0; i <= data.length-1; i++){
            temp = data[i].plat.split(" ")[1]
            
            if(data[i].type == 'Mobil' && temp.slice(-1) % 2 !== 0 && date % 2 == 0 ){
                count++
            }else if(data[i].type == 'Mobil' && temp.slice(-1) % 2 == 0 && date % 2 !== 0){
                count++
            }
        }
    }else{
    ...
    return count
    //grogi anj, syntax dasar ae lupa
  }
